...
// Global interface for React Native
...
class TextDecoderPolyfill implements TextDecoderInterface {
  ...
  decode(input: Uint8Array): string {
    if (this.encoding === 'utf-8' || this.encoding === 'utf8') {
      ...
      while (i < input.length) {
        ...
        if (byte1 !== undefined && byte1 < 0x80) {
          // ASCII character
          result += String.fromCharCode(byte1);
        } else if (byte1 !== undefined && (byte1 & 0xE0) === 0xC0) {
        ...
        } else if (byte1 !== undefined && (byte1 & 0xF0) === 0xE0) {
...
if (typeof global.TextDecoder === 'undefined') {
  global.TextDecoder = TextDecoderPolyfill as unknown as TextDecoderConstructor;
}