...
export class NfcManagerService {
    ...
    try {
      ...
      for (let sector = 0; sector < totalSectors; sector++) {
        ...
        try {
          ...
          const authenticated = await this.authenticateWithKeyA(
            sector,
            keys.keyA,
          );
          if (authenticated) {
  ...
  private deriveKeysFromUid(uid: string): {keyA: number[]; keyB: number[]} {
    ...
    const keyA = uidBytes
      .concat([0xff, 0xff, 0xff, 0xff, 0xff, 0xff])
      .slice(0, 6);
    const keyB = uidBytes
      .concat([0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
      .slice(0, 6);
