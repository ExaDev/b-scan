...
fun FilamentReelDetailsScreen(
    ...
    var spoolDetails by remember { mutableStateOf<FilamentReelDetails?>(null) }
    var isLoading by remember { mutableStateOf(true) }
    
    LaunchedEffect(trayUid) {
        try {
            spoolDetails = repository.getFilamentReelDetails(trayUid)
        } catch (e: Exception) {
            spoolDetails = null
        } finally {
            isLoading = false
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Inventory Stock Details") },
                navigationIcon = {
            ...
            spoolDetails?.let { details ->
                ...
                Box(
                    ...
                    Column(horizontalAlignment = androidx.compose.ui.Alignment.CenterHorizontally) {
                        Text(
                            text = "Inventory Stock not found",
                            style = MaterialTheme.typography.headlineSmall,
                            color = MaterialTheme.colorScheme.error
                        )
                        Spacer(modifier = Modifier.height(8.dp))
                        Text(
                            text = "Could not find inventory stock with tray UID: $trayUid",
